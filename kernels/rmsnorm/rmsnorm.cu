...
#ifdef TORCH_COMPILE
#define TK_COMPILE_FUSED_RMSNORM
#endif
...
__global__ __launch_bounds__(NUM_THREADS, 1) void rmsnorm_tk(
  ...
  vec_smem_1xD(&x_s)[2][NUM_WORKERS] =
      al.allocate<vec_smem_1xD, 2, NUM_WORKERS>();
  vec_smem_1xD(&residual_s)[2][NUM_WORKERS] =
      al.allocate<vec_smem_1xD, 2, NUM_WORKERS>();
  ...
  bf16 rms = __float2bfloat16(0.0f);

  load_async(x_s[warpid][tic], g.x, {batch, 0, seq_start + warpid, 0});
  load_async(residual_s[warpid][tic], g.residual,
             {batch, 0, seq_start + warpid, 0});
  __syncthreads();
  ...
  for (int block = 0; block < n_blocks; block++, tic ^= 1, toc ^= 1) {
    ...
    if (block < n_blocks - 1) {
      ...
      load_async(residual_s[warpid][toc], g.residual,
                 {batch, 0, seq_start + next_idx, 0});
    }
    load_async_wait();
    __syncwarp();
    ...
    store(g.o_resid, residual_s[warpid][tic],
          {batch, 0, seq_start + cur_idx, 0});
    __syncwarp();

    // RMSNorm computation
    mul(x_s[warpid][tic], residual_s[warpid][tic], residual_s[warpid][tic]);
    sum(rms, x_s[warpid][tic]);
    rms = rms / __float2bfloat16(d_model);
    rms =
        __float2bfloat16(sqrt(__bfloat162float(rms + __float2bfloat16(1e-5f))));

    // Apply RMSNorm
    div(residual_s[warpid][tic], residual_s[warpid][tic], rms);
...
void dispatch_rmsnorm(bf16 *d_x_bf, bf16 *d_residual_bf, bf16 *d_norm_weight_bf,
  ...
  globals g{x_arg,           residual_arg, o_arg,     o_resid_arg,
            norm_weight_arg, n_tile_size,  n_per_tile};

  unsigned long mem_size = 24456; // Reduced memory size due to removal of bias
  cudaFuncSetAttribute(rmsnorm_tk<D>,
                       cudaFuncAttributeMaxDynamicSharedMemorySize, mem_size);

  dim3 grid(n_tile_size, B, 1);
  rmsnorm_tk<D><<<grid, NUM_THREADS, mem_size>>>(g, n_per_tile);
...
fused_rmsnorm(const torch::Tensor x, const torch::Tensor residual,
              const torch::Tensor norm_weight, float dropout_p) {
  CHECK_INPUT(x);
  ...
  TORCH_CHECK(x.size(1) % kittens::TILE_ROW_DIM<bf16> == 0,
              "sequence length is divisible by 16?");
  TORCH_CHECK(residual.size(1) % kittens::TILE_ROW_DIM<bf16> == 0,
              "sequence length is divisible by 16?");
  ...
  bf16 *d_o_resid =
      reinterpret_cast<bf16 *>(out_resid.data_ptr<c10::BFloat16>());

  dispatch_rmsnorm(d_x_bf, d_residual_bf, d_norm_weight_bf, d_o, d_o_resid,
                   dropout_p, b, n);
